...
try {
    ...
    if (!isset($input['facebook_token'])) {
        ResponseUtils::error('페이스북 토큰이 필요합니다.');
    }
    
    $facebookToken = trim($input['facebook_token']);
    $username = isset($input['username']) ? trim($input['username']) : null;
    
    // 페이스북 토큰 검증 및 사용자 정보 가져오기
    $facebookUser = verifyFacebookToken($facebookToken);
    
    if (!$facebookUser) {
        ResponseUtils::error('유효하지 않은 페이스북 토큰입니다.');
    }
    
    // 이메일로 기존 사용자 확인
    $existingUser = getUserByEmail($db, $facebookUser['email']);
    
    if ($existingUser) {
        // 기존 사용자 로그인
        if ($existingUser['provider'] !== 'facebook' && $existingUser['provider'] !== 'local') {
            ResponseUtils::error('이 이메일은 다른 소셜 계정으로 가입되어 있습니다.');
        }
        
        // 프로바이더 정보 업데이트
        updateUserProvider($db, $existingUser['id'], 'facebook', $facebookUser['id']);
        
        $user = $existingUser;
    } else {
        // 새 사용자 등록
        if (!$username) {
            // 사용자명이 제공되지 않은 경우 페이스북 이름에서 생성
            $username = generateUsernameFromName($db, $facebookUser['name']);
        } else {
            // 사용자명 유효성 검사
        ...
        $user = createFacebookUser($db, $facebookUser, $username);
    }
...
function verifyFacebookToken($token) {
    try {
        ...
        $url = FACEBOOK_USERINFO_URL . '&access_token=' . $token;
        ...
        return [
            ...
            'name' => $userData['name'] ?? '',
            'picture' => isset($userData['picture']['data']['url']) ? $userData['picture']['data']['url'] : null
        ];
 ...
 * 페이스북 사용자 생성
 */
function createFacebookUser($db, $facebookUser, $username) {
    try {
        $stmt = $db->prepare("
            INSERT INTO users (
        ...
        throw new Exception('페이스북 사용자 생성 실패: ' . $e->getMessage());
    }
}